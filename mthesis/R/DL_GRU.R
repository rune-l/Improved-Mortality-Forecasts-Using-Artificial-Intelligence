...
f_GRU_recursive_cohort <- function(lSplit,
                                   sPython_path,
                                   iBatch_size = 1024,
                                   sSave_path,
                                   iN_epochs = 200,
  ...
  # GRU for Age #
  GRU_age = Age_embed %>% keras::layer_gru(128, activation = "relu",
                                           dropout = iDrop_rate, recurrent_dropout = iDrop_rate)
  
  # GRU for Country #
  GRU_country = Country_embed %>% keras::layer_gru(128, activation = "relu",
                                                   dropout = iDrop_rate, recurrent_dropout = iDrop_rate)
  
  # GRU for gender #
  GRU_gender = Gender_embed %>% keras::layer_gru(128, activation = "relu",
                                                 dropout = iDrop_rate, recurrent_dropout = iDrop_rate)
  
  # GRU for cohort #
  GRU_cohort = Cohort_embed %>% keras::layer_gru(128, activation = "relu",
                                                 dropout = iDrop_rate, recurrent_dropout = iDrop_rate)
  
  # Define the features #
  vFeatures = keras::layer_concatenate(list(Year, GRU_age, GRU_country, GRU_gender, value, GRU_cohort))
